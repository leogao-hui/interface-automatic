...
class TestGetEvaluateAndVariable(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.url = '%s/api/rule/solutionEntityModel/Vars' % ci_url
        ...
        cls.specific_calculation = cls.add_func.add_solution_group_solution_calculationVariable(
            project_group_specific_calculation_variable_one_data)
        ...
        cls.add_func.add_solution_group_solution_calculationVariable(
            project_group_specific_calculation_variable_two_data)
        ...
        cls.add_func.add_solution_group_solution_displayVariable(project_group_specific_display_variable_one_data)
        ...
        cls.add_func.add_solution_group_solution_displayVariable(project_group_specific_display_variable_two_data)
        ...
        cls.add_func.add_solution_group_solution_displayVariable(project_group_specific_display_variable_three_data)
        ...
        cls.assert_solution_variable = ['方案组专用计算变量二', '方案组专用显示变量一', '方案组专用显示变量二', '方案组专用显示变量三', '方案组专用计算变量三', '方案组专用计算变量一']

    # 测试方案组成和方案评定的变量解析
    def test_01(cls):
        data = {
            'evId': cls.evVersionId_one,
            'items[0][solutionEntityModelType]': 'solution_composition',
            'items[0][solutionEntityModelId]': cls.solutionCompositionId_one,
            'items[1][solutionEntityModelType]': 'solution_composition',
            'items[1][solutionEntityModelId]': cls.solutionCompositionId_two,
            'items[2][solutionEntityModelType]': 'solution_assessment',
            'items[2][solutionEntityModelId]': cls.solutionAssessmentId_one,
            'items[3][solutionEntityModelType]': 'solution_assessment',
            'items[3][solutionEntityModelId]': cls.solutionAssessmentId_two,
        }
        response = cls.session.post(url=cls.url, data=data)
        print(response.json())
        for key in response.json()['items']:
            cls.assertIn(key['itemName'], cls.assert_solution_variable)
