...
class TestAddSolutionComposition(unittest.TestCase):
    ...
    # 正常新增方案组成
    def test_add_solution_composition(cls):
        data = {
            'name': '测试方案组成',
    ...
    # 新增方案组成name为空
    def test_add_solution_composition_name_null(cls):
        data = {
            'name': '',
            'solutionItemId': cls.solutionItemId_one,
            'conditionExpression': '*',
    ...
    # 新增方案组成solutionItemId为空
    def test_add_solution_composition_solutionItemId_null(cls):
        data = {
            'name': '测试',
            'solutionItemId': '',
            'conditionExpression': '*',
        ...
        case_name = '新增方案组成solutionItemId为空'
        response = cls.session.post(url=cls.url, data=data)
        if response.status_code == 422:
            println(1, case_name)
            cls.assertEqual(422, response.status_code, response.json())
    ...
    # 新增方案组成solutionItemId不存在
    def test_add_solution_composition_solutionItemId_not_exist(cls):
        data = {
            'name': '测试',
    ...
    # 新增方案组成conditionExpression为空
    def test_add_solution_composition_conditionExpression_null(cls):
        data = {
            'name': '测试',
            'solutionItemId': cls.solutionItemId_one,
            'conditionExpression': '',
    ...
    def test_add_solution_composition_calculation_variable_scheme_available_group_calculation(cls):
        ...
        case_name = '方案组成显示条件中可以使用方案组专用计算变量'
        response = cls.session.post(url=cls.url, data=data)
        if response.status_code == 200:
            println(1, case_name)
            cls.assertEqual(200, response.status_code)
    ...
    def test_add_solution_composition_scale_variable_conventional(cls):
        data = {
            'name': '测试方案组成常规量表变量',
            'solutionItemId': cls.solutionItemId_one,
            'conditionExpression': '量表变量一>70',
            'displayOrder': 1,
            'content': '[{"name": "方案组成结果1", "displayContent": "具体显示内容", "group": "1", "comment": "备注",\
                         "conditions": [{"expression": "(职业$1(专业技术人员))", "presentation": "情况说明", "comment": "备注"},\
                                        {"expression": "(职业$1(休养或退休))", "presentation": "情况说明", "comment": "备注"}]},\
                        {"name": "方案组成结果2", "displayContent": "具体显示内容1", "group": "2", "comment": "备注",\
                         "conditions": [{"expression": "(职业$1(专业技术人员))", "presentation": "情况说明", "comment": "备注"},\
                                        {"expression": "(职业$1(休养或退休))", "presentation": "情况说明", "comment": "备注"}]}]',
            'comment': ''
        }
    ...
    def test_add_solution_composition_scale_variable_scheme_available_group(cls):
        ...
        case_name = '方案组成显示条件中不可以使用方案组可用量表变量'
        response = cls.session.post(url=cls.url, data=data)
        if response.status_code == 200:
            println(1, case_name)
            cls.assertEqual(200, response.status_code)
    ...
    def test_add_solution_composition_evaluate_conventional(cls):
        data = {
            'name': '测试方案组成常规评估项',
            'solutionItemId': cls.solutionItemId_one,
            'conditionExpression': '测试评估名1=匹配用结果项名1',
            'displayOrder': 1,
            'content': '[{"name": "方案组成结果1", "displayContent": "具体显示内容", "group": "1", "comment": "备注",\
                         "conditions": [{"expression": "(职业$1(专业技术人员))", "presentation": "情况说明", "comment": "备注"},\
                                        {"expression": "(职业$1(休养或退休))", "presentation": "情况说明", "comment": "备注"}]},\
    ...
    def test_add_solution_composition_not_have_this(cls):
        data = {
            'name': '测试方案组成方案组可用评估项',
            'solutionItemId': cls.solutionItemId_one,
            'conditionExpression': 'this=匹配用结果项名1',
            'displayOrder': 1,
            'content': '[{"name": "方案组成结果1", "displayContent": "具体显示内容", "group": "1", "comment": "备注",\
                                 "conditions": [{"expression": "(职业$1(专业技术人员))", "presentation": "情况说明", "comment": "备注"},\
                                                {"expression": "(职业$1(休养或退休))", "presentation": "情况说明", "comment": "备注"}]},\
    ...
    def test_add_solution_composition_have_food_nutrients(cls):
        data = {
            'name': '测试方案组成方案组可用评估项',
            'solutionItemId': cls.solutionItemId_one,
            'conditionExpression': '(GI值=10)&(状态=固态)',
            'displayOrder': 1,
            'content': '[{"name": "方案组成结果1", "displayContent": "具体显示内容", "group": "1", "comment": "备注",\
                                         "conditions": [{"expression": "(职业$1(专业技术人员))", "presentation": "情况说明", "comment": "备注"},\
                                                        {"expression": "(职业$1(休养或退休))", "presentation": "情况说明", "comment": "备注"}]},\
    ...
    def add_solution_composition_expression_not_have_direct_variable_conventional(cls):
        pass
